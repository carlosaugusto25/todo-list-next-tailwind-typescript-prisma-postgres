...
import { updateAllTask, updateTask } from "./services/editTask";
...
export default function Home() {
  ...
  const editAllMutation = useMutation({
    mutationFn: updateAllTask,
    onSuccess:()=>{
      queryClient.invalidateQueries({ queryKey: ['todos'] });
      toast.success("Reordenação completada!")
    },
    onError:()=>{
  ...
  const editTaskFunction = (title: string, description: string, id: number, category: string, position: number, completedTask: boolean) => {
    setTitle(title)
  ...
  const handleEditTask = () => {
    editMutation.mutate({ id, title, description, category, completed, position })
  }
  ...
  const onDragEnd = (result:DropResult) => {
    ...
    queryClient.setQueryData(['todos'], updatedTasks);

    editAllMutation.mutate(updatedTasks);
      ...
      <div className="p-4  max-w-4xl mx-auto h-[calc(100vh-5rem)]">
        {
          (isLoading || isFetching) ?
            ...
            <DragDropContext onDragEnd={onDragEnd}>
              <Droppable droppableId="todoList" type="list" direction="vertical">
                {
                  (provided) => (
                              ...
                              <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                <div className={`flex w-full ${todo.completed ? 'bg-gray-100 dark:bg-slate-600' : 'bg-gray-200 dark:bg-slate-500'} rounded-md p-4 mb-4 justify-between max-[768px]:flex-col max-[768px]:gap-4`} key={todo.id}>
                                  ...
                                  <div className="flex gap-2 items-center">
                                    ...
                                    {!todo.completed && <button className="bg-blue-500 border-none outline-none rounded-md text-white p-3" onClick={() => editTaskFunction(todo.title, todo.description, todo.id, todo.category, todo.position, todo.completed, )}><FiEdit /></button>}
                                    <button className={`${todo.completed ? 'bg-slate-700' : 'bg-green-500'} border-none outline-none rounded-md ${todo.completed ? 'text-slate-400' : 'text-white'} text-2xl p-2`} onClick={() => editMutation.mutate({ title: todo.title, description: todo.description, id: todo.id, category: todo.category, position: todo.position, completed: !todo.completed })}><IoMdCheckboxOutline /></button>
                                  </div>
                                </div>
                              </div>
                            )
